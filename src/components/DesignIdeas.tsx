...
const filterOptions = ["All", "Cottagecore", "Cosmic", "Japanese Streetwear", "Retro Sports", "Mental Health"];
    ...
    <section className="mb-10">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
        <div>
          ...
          <p className="text-gray-400 text-sm">AI-generated prompts + curated t-shirt concepts ready to create</p>
        </div>
            ...
            >
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-2">
                  <h3 className="text-white font-semibold text-sm leading-tight">{idea.title}</h3>
                  <span className="text-emerald-400 font-bold text-sm ml-2 shrink-0">{idea.sellPrice}</span>
                </div>
                ...
                <div className="bg-gray-800/60 rounded-xl p-3 border border-gray-700/40 mb-3">
                  ...
                  <p className="text-gray-400 text-xs leading-relaxed line-clamp-3">{idea.prompt}</p>
                </div>

                <div className="flex items-center gap-2 mb-3">
                  <div className="flex gap-1">
                    {idea.colors.map((color) => (
                      <div key={color} className="w-5 h-5 rounded-full border-2 border-gray-700/50" style={{ backgroundColor: color }} title={color} />
                    ))}
                  </div>
