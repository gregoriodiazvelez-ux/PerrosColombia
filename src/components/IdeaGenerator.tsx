...
const niches = ["Cottagecore", "Cosmic/Celestial", "Japanese Streetwear", "Dark Academia", "Retro Sports", "Mental Health", "Y2K Nostalgia", "Pet Portraits"];
const styles = ["Vintage Illustration", "Minimalist Line Art", "Bold Graphic", "Watercolor", "Woodcut", "Neon Cyberpunk", "Kawaii Cute", "Typographic"];
const audiences = ["Gen Z Women", "Millennial Men", "Pet Lovers", "Gamers", "Yoga Enthusiasts", "Coffee Addicts", "Plant Parents", "Bookworms"];
...
function generateIdea(niche: string, style: string, audience: string): GeneratedIdea {
  const ideas: Record<string, GeneratedIdea> = {
    ...
    Cosmic: {
      ...
      prompt: `Design a ${style.toLowerCase()} artwork of a wolf made entirely of constellation lines and stars. Background: deep indigo nebula with swirling purple and blue cosmic clouds. Include: crescent moon, scattered stars, and subtle galaxy spiral. For ${audience}.`,
    ...
    Japanese: {
      ...
      prompt: `Create a ${style.toLowerCase()} graphic of a ramen bowl levitating above a neon-lit Tokyo street at night. Include: steam rising with glowing kanji, neon signs in Japanese, cherry blossoms blowing past, chopsticks hovering above bowl. Colors: electric red, neon yellow, deep black. For ${audience}.`,
    ...
    Dark: {
      ...
      concept: "An anatomically detailed death's head moth pinned to aged parchment surrounded by pressed flowers and cryptic latin text",
      prompt: `Design a ${style.toLowerCase()} illustration in Victorian scientific journal style. Central element: Acherontia atropos moth with skull marking, wings spread. Surrounding: pressed botanical specimens (belladonna, foxglove, black roses), aged parchment texture, handwritten latin taxonomy notes. Sepia and ink tones. For ${audience}.`,
    ...
    Mental: {
      ...
      prompt: `Create a ${style.toLowerCase()} illustration of a cracked terracotta pot with golden kintsugi repairs, and a beautiful green plant growing vigorously through the breaks. Below: bold modern typography reading 'Growing Through It'. Colors: warm terracotta, forest green, gold. For ${audience}.`,
  ...
  const nicheKey = Object.keys(ideas).find((k) => niche.startsWith(k));
  if (nicheKey) return ideas[nicheKey];

  return {
    title: `${style} ${niche} Design`,
    concept: `A ${style.toLowerCase()} take on ${niche} themes crafted for ${audience}`,
      ...
      <div className="rounded-2xl border border-violet-500/30 bg-gradient-to-br from-violet-950/50 via-gray-900/80 to-fuchsia-950/50 overflow-hidden">
        <div className="px-6 py-5 border-b border-violet-500/20 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-xs">Select your parameters and get a complete design brief + AI prompt</p>
          </div>
        </div>
        ...
        <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="space-y-5">
            <div>
              <label className="block text-gray-300 text-sm font-semibold mb-2">Niche</label>
            ...
            <div>
              <label className="block text-gray-300 text-sm font-semibold mb-2">Art Style</label>
              <div className="grid grid-cols-2 gap-2">
                {styles.map((s) => (
            ...
            <div>
              <label className="block text-gray-300 text-sm font-semibold mb-2">Target Audience</label>
              <div className="grid grid-cols-2 gap-2">
                {audiences.map((a) => (
          ...
          <div className="relative min-h-64">
            {!generated && !loading && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-8">
                ...
                <p className="text-gray-400 text-sm">Select your parameters and click Generate to get a complete design brief</p>
              </div>
            )}
            ...
            {loading && (
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <div className="relative w-16 h-16">
                  ...
                  <div className="absolute inset-2 rounded-full border-2 border-fuchsia-500/20 border-t-fuchsia-500 animate-spin" style={{ animationDirection: "reverse" }} />
                </div>
                ...
                <div className="bg-gray-900/60 rounded-xl border border-gray-700/50 p-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {generated.hashtags.map((tag) => (
                      <span key={tag} className="text-xs bg-gray-800 text-gray-300 px-2.5 py-1 rounded-lg border border-gray-700/50">{tag}</span>
                    ))}
                  </div>
                </div>
              </div>
