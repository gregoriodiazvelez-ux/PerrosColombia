...
const tips = [
  {
    icon: Target, color: "text-violet-400", bg: "bg-violet-500/10 border-violet-500/20",
  ...
  {
    icon: Lightbulb, color: "text-fuchsia-400", bg: "bg-fuchsia-500/10 border-fuchsia-500/20",
    ...
    <section className="mb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {tips.map((tip) => {
          const Icon = tip.icon;
          return (
            <div key={tip.title} className={`rounded-2xl border p-5 ${tip.bg}`}>
              <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-3 ${tip.bg}`}>
