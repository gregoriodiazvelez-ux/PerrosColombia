  ...
  return (
    <section className="mb-10">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-bold text-white mb-1">Trending Niches</h2>
          <p className="text-gray-400 text-sm">Top performing categories for print-on-demand this week</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {trendingNiches.map((niche) => (
          ...
          >
            ...
            <h3 className="text-white font-semibold text-base mb-1 leading-tight">{niche.name}</h3>
            <span
              className="inline-block text-xs px-2 py-0.5 rounded-md mb-3 font-medium"
              style={{ color: niche.color, backgroundColor: `${niche.color}18` }}
            >
              {niche.category}
            </span>
            <p className="text-gray-400 text-xs leading-relaxed mb-4">{niche.description}</p>
            ...
            <div className="flex flex-wrap gap-1.5 mt-3">
              {niche.tags.slice(0, 2).map((tag) => (
                <span key={tag} className="flex items-center gap-1 text-xs text-gray-500 bg-gray-800 px-2 py-0.5 rounded-md border border-gray-700/50">
